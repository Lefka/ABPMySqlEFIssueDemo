...
namespace ABPEFTest.Tests
{
    public abstract class ABPEFTestTestBase : AbpIntegratedTestBase<ABPEFTestTestModule>
    {
        ...
        protected void UsingDbContext(Action<ABPEFTestDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ABPEFTestDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ABPEFTestDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ABPEFTestDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ABPEFTestDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ABPEFTestDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ABPEFTestDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ABPEFTestDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ABPEFTestDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ABPEFTestDbContext, Task<T>> func)
        {
            T result;
